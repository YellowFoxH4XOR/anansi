...
| 0:40 | **Kill shot, compressed** | Fire M2 on camera. One product of four switches to the promo template. The run reports SUCCESS — healthy success rate, no error code, three perfect rows — and ANANSI opens an incident on the one that came back null. Twenty seconds, no mechanism yet: a job-level health check cannot see this by construction, and every lookalike is now behind us. **Deterministic: Sense-only, no heal, no LLM — bankable the moment Sense works (D2 night), not D4.** |
