...
| `tag_html()` snapshots | Tagged on every page load and explicitly `collect()`ed into a `_snapshot_html` output field (tagged values reach parser code only — collecting is on us); the stored last-good/current pair feeds the DOM diff | [scraper/lab-scraper.js](scraper/lab-scraper.js), [core/diagnose/](core/diagnose/) |
