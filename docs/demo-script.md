...
| 0:40 | **Kill shot, compressed** | Fire L3 on camera. The index's product links go JS-driven: every card still matches `a.card-link`, and `new URL("#", base)` resolves every one to the index. Stage 2 scrapes the same page four times — the right NUMBER of rows, all of them the wrong page, and SUCCESS reported throughout. A row-count check passes; the DOM diff is empty; the product pages are all still perfect. Twenty seconds, no mechanism yet — every lookalike is now behind us. **Deterministic: Sense-only, no heal, no LLM.** |
...
| 1:50 | **Heal and gate** | Open with 3–4 s of the **Scraper Studio IDE itself** (the hand-authored scraper, `tag_html`/`collect` visible — "their healer, our gate" lands harder with their product on screen), then: proposed diff → V1 preview check against goldens → approve → **the collector goes to `watching`, and Bright Data's own next scheduled run is the verification** (narrate the gate honestly — the preview is backend-chosen sample rows, and a judge who reads the CLI docs knows it; say plainly that ANANSI does not re-run anything to confirm its own fix, [ADR-005](decisions/005-verify-v2-dropped.md)). Real elapsed time as a labelled cut ("11 min later"). The honest timestamp is a credibility asset. |
